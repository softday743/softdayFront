            ...
            <div className={`checklist-item ${selected.includes('video') ? 'selected' : ''}`} onClick={() => toggleSelection('video')}>
                <div className={`checkbox ${selected.includes('video') ? 'checked' : ''}`}>
                    {selected.includes('video') && (
                        <svg width="17" height="17" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }}>
                            <path d="M14.1673 4.25L6.37565 12.0417L2.83398 8.5" stroke="#F6F6F6" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                        </svg>
                    )}
                </div>
