  ...
  const handleComplete = async () => {
    ...
    try {
      await api.post("/check-in", {
        stressLevel: mood, // ⚠️ 백엔드 DTO(CheckInRequest) 필드명에 맞춤 (score -> stressLevel)
        stressCauses: selectedReasons, // ⚠️ 백엔드 필드명에 맞춤 (causes -> stressCauses)
        memo: selectedReasons.includes("기타") ? otherReason : "", // ⚠️ 백엔드 필드명에 맞춤 (note -> memo)
      });
          ...
          <div className="sc-tags-container">
            {reasons.map((r) => (
              <div
                key={r}
                className={`sc-tag ${
                  selectedReasons.includes(r) ? "selected" : ""
                } ${r === "기타" ? "wide" : ""}`}
        ...
        <div className="sc-overlay">
          <div className="sc-popup">
            ...
            <div className="sc-popup-btn-group">
              <div className="sc-popup-btn cancel" onClick={onBack}>
                그만하기
              </div>
              <div
                className="sc-popup-btn confirm"
